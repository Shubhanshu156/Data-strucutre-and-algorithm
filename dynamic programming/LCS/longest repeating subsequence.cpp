...
int topdown(string x,string y,int n,int m){
    ...
    for (int i = 1; i <= n; i++)
    {
        ...
        for (int j = 1; j <= m; j++)
        {
            if (x[i-1]==y[j-1] &&(i!=j)) //same as lcs add only if index is diffrent string1=AABEBCDD we are adding
                                         //CHECK PAINT IMAGE FOR BETTER UNDERSTANDING
            {
                DP[i][j]=DP[i-1][j-1]+1;
...
int main(){
    ...
    string x="AABEBCDD";
    string y="AABEBCDD";
    cout<<topdown(x,y,8,8); //so first we are passsing this as both string
}